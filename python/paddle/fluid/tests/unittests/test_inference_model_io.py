    ...
    def test_errors(self):
        with program_guard(Program(), Program()):
            def test_dirname_bool():
                fluid.io.load_inference_model(dirname=True,
                    executor=executor.Executor(core.CPUPlace()))
            def test_executor_Variable():
                fluid.io.load_inference_model(dirname=str(),
                    executor=fluid.framework.Variable())
            ...
            def test_pserver_endpoints_Variable_list():
                fluid.io.load_inference_model(dirname=str(),
                    executor=executor.Executor(core.CPUPlace()),
                    pserver_endpoints=(fluid.framework.Variable()))
